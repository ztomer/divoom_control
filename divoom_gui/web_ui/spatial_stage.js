...
(function() {
    ...
    function initSpatialStage() {
        ...
        header.innerHTML = `
            ...
            <div class="spatial-stage-actions">
                <button id="stage-snap-btn" class="stage-btn" type="button">
                    <svg class="kare-icon" viewBox="0 0 16 16">
                ...
                <button id="stage-collapse-btn" class="stage-btn" type="button">
                    <svg class="kare-icon" viewBox="0 0 16 16">
                        <rect x="2" y="4" width="12" height="8" rx="2" fill="none" stroke="currentColor" stroke-width="1.5"/>
        ...
        inspector.innerHTML = `
            ...
            <div class="spatial-inspector-right">
                <div class="spatial-brightness-control">
                    <svg class="kare-icon" viewBox="0 0 16 16" style="color: var(--text-muted);">
        ...
        // Load topology from local cache
        try {
    ...
    function getDeviceList() {
        ...
        return [
            { address: '11:22:33:44:55:01', name: 'Desk Left', model: 'Ditoo', size: 16, activityKind: 'clock' },
            { address: '11:22:33:44:55:02', name: 'Desk Mid-L', model: 'Timoo', size: 16, activityKind: 'sysmon' },
            { address: '11:22:33:44:55:03', name: 'Desk Mid-R', model: 'Ditoo', size: 16, activityKind: 'visualizer' },
            { address: '11:22:33:44:55:04', name: 'Desk Right', model: 'Pixoo-64', size: 64, activityKind: 'weather' }
        ];
    }
    ...
    function refreshBenchNodes() {
        ...
        const summary = document.getElementById('stage-fleet-summary');
        if (summary) summary.textContent = `${devices.length} Displays Online`;
        ...
        devices.forEach((dev, idx) => {
            ...
            node.className = `spatial-node form-${spec.form}` + (isSelected ? ' selected' : '');
            ...
            node.innerHTML = `
                <div class="spatial-node-header" style="width: 100%;">
                    <span class="spatial-node-name">${dev.name || spec.name}</span>
                    <span class="spatial-jewel online"></span>
                </div>
    ...
    function updateInspector(dev) {
        ...
        if (inspModel) inspModel.textContent = `${spec.w_mm}×${spec.h_mm}mm (${spec.pw}×${spec.ph})`;
            ...
            devices.forEach((dev, idx) => {
                ...
                if (kind === 'clock') {
                    ...
                    if (Math.floor(tick / 25) % 2 === 0) {
                        ctx.fillRect(9, 7, 1, 1); ctx.fillRect(9, 9, 1, 1);
                    }
                    ...
                    // 64x64 or weather radar
                    if (is64) {
                        ctx.strokeStyle = '#1e293b';
                        ctx.strokeRect(6, 6, 52, 52);
                        ctx.fillStyle = '#38bdf8';
                        for (let i = 0; i < 24; i++) {
                            const cx = Math.round(32 + 16 * Math.cos(i * 0.25 + tick * 0.02));
                            const cy = Math.round(32 + 12 * Math.sin(i * 0.3 + tick * 0.02));
