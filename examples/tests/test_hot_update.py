...
from divoom_legacy.tools.hot_update import HotFile, HotUpdate, clear_hot_manifest_cache
...
async def test_session_replays_real_ditoo_exchange(monkeypatch):
    ...
    hu = HotUpdate(divoom)
    monkeypatch.setattr("divoom_legacy.tools.hot_update.fetch_hot_manifest", lambda dt: [f])
    monkeypatch.setattr("divoom_legacy.tools.hot_update.download_hot_file", lambda x: True)
    monkeypatch.setattr("divoom_legacy.tools.hot_update.INTER_PACKET_DELAY", 0)
...
async def test_load_hot_files_caches_per_device_type(monkeypatch):
    ...
    from divoom_legacy.tools import hot_update as hu_mod
