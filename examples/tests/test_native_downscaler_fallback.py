"""PIL-fallback + error-handling coverage for divoom_legacy.native.downscaler
(split from test_native_downscaler.py)."""
import pytest

from divoom_legacy.native import CHANNELS_RGB, downsample_lanczos, is_native_available, reset_for_tests
from support.downscaler_common import (
    SEED_PARITY,
    SIDE_EIGHT,
    SIDE_SMALL,
    SIDE_TINY,
...
class TestPILFallback:
    ...
    def test_fallback_when_dylib_missing(self, monkeypatch):
        ...
        monkeypatch.setattr("divoom_legacy.native.downscaler.ctypes.CDLL", _cdll_fails)
        reset_for_tests()
