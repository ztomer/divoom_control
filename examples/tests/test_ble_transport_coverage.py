...
The ONLY correct patch target is ``divoom_legacy.divoom.BleakClient`` — patching
``divoom_legacy.ble_transport.BleakClient`` silently does nothing, and a missed
...
patches ``divoom_legacy.divoom.BleakClient`` before constructing a transport with
...
from divoom_legacy.ble_transport import BLETransport
...
def _transport(monkeypatch, **cfg_kwargs):
    ...
    with patch("divoom_legacy.divoom.BleakClient") as mk:
