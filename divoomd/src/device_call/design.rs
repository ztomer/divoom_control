use super::CallCtx;
use crate::protocol::err_reply;
use crate::wire::WireNarrow as _;
use serde_json::{json, Value};

pub async fn handle(method: &str, ctx: CallCtx<'_>) -> Value {
    match method {
        "design.set_eq" | "set_eq" => set_eq(ctx).await,
        "design.set_language" | "set_language" => set_language(ctx).await,
        "design.set_user_define_time" | "set_user_define_time" => set_user_define_time(ctx).await,
        "design.get_user_define_time" | "get_user_define_time" => get_user_define_time(ctx).await,
        "design.set_screen_dir" | "set_screen_dir" => set_screen_dir(ctx).await,
        "design.set_screen_mirror" | "set_screen_mirror" => set_screen_mirror(ctx).await,
        "design.factory_reset" | "factory_reset" => factory_reset(ctx).await,
        "design.use_user_define_index" | "use_user_define_index" => {
            use_user_define_index(ctx).await
        }
        "design.clear_user_define_index" | "clear_user_define_index" => {
            clear_user_define_index(ctx).await
        }
        _ => err_reply("unimplemented design command"),
    }
}
async fn set_eq(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let raw_args = ctx.raw_args;
    let kw = ctx.kwargs;

    let dynamic = kw
        .and_then(|v| v.get("dynamic"))
        .and_then(serde_json::Value::as_bool)
        .or_else(|| raw_args.first().and_then(serde_json::Value::as_bool))
        .unwrap_or(false);
    // R67/C7: `dynamic` is a bool at position 0, which the numeric list
    // drops — so args[1] was the SECOND number, not `mode`.
    let mode = crate::device_call::pos_i64(raw_args, 1, kw, "mode", 0).byte();
    let stream = kw
        .and_then(|v| v.get("stream"))
        .and_then(serde_json::Value::as_bool)
        .or_else(|| raw_args.get(2).and_then(serde_json::Value::as_bool))
        .unwrap_or(false);
    let payload = [0x1eu8, u8::from(dynamic), mode, u8::from(stream)];
    match dev.send_command(0xbd, &payload, true).await {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("set_eq failed: {e}")),
    }
}

async fn set_language(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;

    let lang = args
        .first()
        .copied()
        .or_else(|| {
            kw.and_then(|v| v.get("lang"))
                .and_then(serde_json::Value::as_i64)
        })
        .unwrap_or(0)
        .byte();
    match dev.send_command(0xbd, &[0x26, lang], true).await {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("set_language failed: {e}")),
    }
}

async fn set_user_define_time(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;

    let hour = args
        .first()
        .copied()
        .or_else(|| {
            kw.and_then(|v| v.get("hour"))
                .and_then(serde_json::Value::as_i64)
        })
        .unwrap_or(0)
        .byte();
    let minute = args
        .get(1)
        .copied()
        .or_else(|| {
            kw.and_then(|v| v.get("minute"))
                .and_then(serde_json::Value::as_i64)
        })
        .unwrap_or(0)
        .byte();
    let second = args
        .get(2)
        .copied()
        .or_else(|| {
            kw.and_then(|v| v.get("second"))
                .and_then(serde_json::Value::as_i64)
        })
        .unwrap_or(0)
        .byte();
    match dev
        .send_command(0xbd, &[0x14, hour, minute, second], true)
        .await
    {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("set_user_define_time failed: {e}")),
    }
}

async fn get_user_define_time(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let timeout = ctx.timeout;

    match dev.send_command_and_wait(0xbd, &[0x15], timeout).await {
        Some(p) if p.len() >= 3 => json!({
            "success": true,
            "result": {
                "hour": i64::from(p[0]),
                "minute": i64::from(p[1]),
                "second": i64::from(p[2]),
            }
        }),
        Some(p) if p.len() == 2 => json!({
            "success": true,
            "result": {
                "hour": i64::from(p[0]),
                "minute": i64::from(p[1]),
                "second": 0,
            }
        }),
        _ => json!({"success": true, "result": Value::Null}),
    }
}

async fn set_screen_dir(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;

    let direction = args
        .first()
        .copied()
        .or_else(|| {
            kw.and_then(|v| v.get("direction"))
                .and_then(serde_json::Value::as_i64)
        })
        .unwrap_or(0)
        .byte();
    match dev.send_command(0xbd, &[0x23, direction], true).await {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("set_screen_dir failed: {e}")),
    }
}

async fn set_screen_mirror(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let raw_args = ctx.raw_args;
    let kw = ctx.kwargs;

    let on = kw
        .and_then(|v| v.get("on"))
        .and_then(serde_json::Value::as_bool)
        .or_else(|| {
            kw.and_then(|v| v.get("enabled"))
                .and_then(serde_json::Value::as_bool)
        })
        .or_else(|| raw_args.first().and_then(serde_json::Value::as_bool))
        .unwrap_or(false);
    match dev.send_command(0xbd, &[0x24, u8::from(on)], true).await {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("set_screen_mirror failed: {e}")),
    }
}

async fn factory_reset(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;

    match dev.send_command(0xbd, &[0x25, 0x01], true).await {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("factory_reset failed: {e}")),
    }
}

async fn use_user_define_index(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;

    let page = args
        .first()
        .copied()
        .or_else(|| {
            kw.and_then(|v| v.get("page"))
                .and_then(serde_json::Value::as_i64)
        })
        .unwrap_or(0)
        .byte();
    match dev.send_command(0xbd, &[0x17, page], true).await {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("use_user_define_index failed: {e}")),
    }
}

async fn clear_user_define_index(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;

    let page = args
        .first()
        .copied()
        .or_else(|| {
            kw.and_then(|v| v.get("page"))
                .and_then(serde_json::Value::as_i64)
        })
        .unwrap_or(0)
        .byte();
    match dev.send_command(0xbd, &[0x16, page], true).await {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("clear_user_define_index failed: {e}")),
    }
}
