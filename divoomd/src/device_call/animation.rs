...
pub async fn handle(method: &str, ctx: CallCtx<'_>) -> Value {
    ...
    match method {
        "animation.set_gif_speed" | "set_gif_speed" => set_gif_speed(ctx).await,
        "animation.set_light_phone_gif" | "set_light_phone_gif" => set_light_phone_gif(ctx).await,
        ...
        "animation.app_get_user_define_info" | "app_get_user_define_info" => {
            app_get_user_define_info(ctx).await
        }
        _ => err_reply("unimplemented animation command"),
    }
}
async fn set_gif_speed(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
...
async fn set_light_phone_gif(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
...
async fn app_new_send_gif_cmd(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    // file_data/big chunk may arrive as blob[0]; prefer it over a kwargs array.
    let blob0 = ctx.blob_map.lock().unwrap().get(&0).cloned();
    let file_data = || -> Vec<u8> { blob0.clone().unwrap_or_else(|| kw_bytes(kw, "file_data")) };
    let cw = || -> i64 {
        args.first()
            .copied()
...
async fn set_user_gif(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    let cw = || -> i64 {
...
async fn app_new_user_define(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    // file_data/big chunk may arrive as blob[0]; prefer it over a kwargs array.
...
async fn app_big64_user_define(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    // file_data/big chunk may arrive as blob[0]; prefer it over a kwargs array.
...
async fn modify_user_gif_items(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
...
async fn app_get_user_define_info(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
