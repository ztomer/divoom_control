//! SD-card music control — parity port of `divoom_lib/media/music.py` (the
//! SD/play methods not already in basic.rs).
//!
//! Setters + read-backs; command ids + response offsets taken verbatim from the
//! Python source.

use crate::wire::le16;
use crate::wire::WireNarrow as _;
use serde_json::{json, Map, Value};

use super::CallCtx;
use crate::protocol::err_reply;

fn kw_i64(kw: Option<&Map<String, Value>>, name: &str) -> Option<i64> {
    kw.and_then(|m| m.get(name))
        .and_then(serde_json::Value::as_i64)
}

pub async fn handle(method: &str, ctx: CallCtx<'_>) -> Value {
    match method {
        // ── setters ────────────────────────────────────────────────────────
        "music.app_need_get_music_list" | "app_need_get_music_list" => {
            app_need_get_music_list(ctx).await
        }
        "music.send_sd_list_over" | "send_sd_list_over" => send_sd_list_over(ctx).await,
        "music.set_play_status" | "set_play_status" => set_play_status(ctx).await,
        "music.set_sd_last_next" | "set_sd_last_next" => set_sd_last_next(ctx).await,
        "music.set_sd_music_play_mode" | "set_sd_music_play_mode" => {
            set_sd_music_play_mode(ctx).await
        }
        "music.set_sd_music_position" | "set_sd_music_position" => set_sd_music_position(ctx).await,
        "music.set_sd_play_music_id" | "set_sd_play_music_id" => set_sd_play_music_id(ctx).await,
        "music.set_sd_music_info" | "set_sd_music_info" => set_sd_music_info(ctx).await,

        // ── read-backs ─────────────────────────────────────────────────────
        "music.get_play_status" | "get_play_status" => get_play_status(ctx).await,
        "music.get_sd_music_list_total_num" | "get_sd_music_list_total_num" => {
            get_sd_music_list_total_num(ctx).await
        }
        "music.get_sd_music_info" | "get_sd_music_info" => get_sd_music_info(ctx).await,
        "music.get_sd_play_name" | "get_sd_play_name" => get_sd_play_name(ctx).await,
        "music.get_sd_music_list" | "get_sd_music_list" => get_sd_music_list(ctx).await,
        _ => err_reply("unimplemented music command"),
    }
}
async fn app_need_get_music_list(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let ok = |r: Result<(), String>, label: &str| match r {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("{label} failed: {e}")),
    };

    ok(
        dev.send_command(0x47, &[], true)
            .await
            .map_err(|e| e.to_string()),
        "app_need_get_music_list",
    )
}

async fn send_sd_list_over(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let ok = |r: Result<(), String>, label: &str| match r {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("{label} failed: {e}")),
    };

    ok(
        dev.send_command(0x14, &[], true)
            .await
            .map_err(|e| e.to_string()),
        "send_sd_list_over",
    )
}

async fn set_play_status(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    let first_arg = |name: &str| args.first().copied().or_else(|| kw_i64(kw, name));
    let ok = |r: Result<(), String>, label: &str| match r {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("{label} failed: {e}")),
    };

    let s = first_arg("status").unwrap_or(0).byte();
    ok(
        dev.send_command(0x0a, &[s], true)
            .await
            .map_err(|e| e.to_string()),
        "set_play_status",
    )
}

async fn set_sd_last_next(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    let first_arg = |name: &str| args.first().copied().or_else(|| kw_i64(kw, name));
    let ok = |r: Result<(), String>, label: &str| match r {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("{label} failed: {e}")),
    };

    let a = first_arg("action").unwrap_or(0).byte();
    ok(
        dev.send_command(0x12, &[a], true)
            .await
            .map_err(|e| e.to_string()),
        "set_sd_last_next",
    )
}

async fn set_sd_music_play_mode(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    let first_arg = |name: &str| args.first().copied().or_else(|| kw_i64(kw, name));
    let ok = |r: Result<(), String>, label: &str| match r {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("{label} failed: {e}")),
    };

    let pm = first_arg("play_mode").unwrap_or(0).byte();
    ok(
        dev.send_command(0xb9, &[pm], true)
            .await
            .map_err(|e| e.to_string()),
        "set_sd_music_play_mode",
    )
}

async fn set_sd_music_position(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    let first_arg = |name: &str| args.first().copied().or_else(|| kw_i64(kw, name));
    let ok = |r: Result<(), String>, label: &str| match r {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("{label} failed: {e}")),
    };

    let pos = first_arg("position").unwrap_or(0);
    ok(
        dev.send_command(0xb8, &le16(pos), true)
            .await
            .map_err(|e| e.to_string()),
        "set_sd_music_position",
    )
}

async fn set_sd_play_music_id(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    let first_arg = |name: &str| args.first().copied().or_else(|| kw_i64(kw, name));
    let ok = |r: Result<(), String>, label: &str| match r {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("{label} failed: {e}")),
    };

    let id = first_arg("music_id").unwrap_or(0);
    ok(
        dev.send_command(0x11, &le16(id), true)
            .await
            .map_err(|e| e.to_string()),
        "set_sd_play_music_id",
    )
}

async fn set_sd_music_info(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    let ok = |r: Result<(), String>, label: &str| match r {
        Ok(()) => json!({"success": true, "result": true}),
        Err(e) => err_reply(&format!("{label} failed: {e}")),
    };

    let cur = kw_i64(kw, "current_time")
        .or_else(|| args.first().copied())
        .unwrap_or(0);
    let mid = kw_i64(kw, "music_id")
        .or_else(|| args.get(1).copied())
        .unwrap_or(0);
    let vol = kw_i64(kw, "volume")
        .or_else(|| args.get(2).copied())
        .unwrap_or(0)
        .byte();
    let st = kw_i64(kw, "status")
        .or_else(|| args.get(3).copied())
        .unwrap_or(0)
        .byte();
    let pm = kw_i64(kw, "play_mode")
        .or_else(|| args.get(4).copied())
        .unwrap_or(0)
        .byte();
    let mut p = Vec::new();
    p.extend_from_slice(&le16(cur));
    p.extend_from_slice(&le16(mid));
    p.push(vol);
    p.push(st);
    p.push(pm);
    ok(
        dev.send_command(0xb5, &p, true)
            .await
            .map_err(|e| e.to_string()),
        "set_sd_music_info",
    )
}

async fn get_play_status(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let to = ctx.timeout;

    match dev.send_command_and_wait(0x0b, &[], to).await {
        Some(r) if !r.is_empty() => json!({"success": true, "result": i64::from(r[0])}),
        _ => json!({"success": true, "result": Value::Null}),
    }
}

async fn get_sd_music_list_total_num(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let to = ctx.timeout;

    match dev.send_command_and_wait(0x7d, &[], to).await {
        Some(r) if r.len() >= 2 => {
            json!({"success": true, "result": i64::from(u16::from_le_bytes([r[0], r[1]]))})
        }
        _ => json!({"success": true, "result": Value::Null}),
    }
}

async fn get_sd_music_info(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let to = ctx.timeout;

    match dev.send_command_and_wait(0xb4, &[], to).await {
        Some(r) if r.len() >= 9 => json!({"success": true, "result": {
            "current_time": u16::from_le_bytes([r[0], r[1]]),
            "total_time": u16::from_le_bytes([r[2], r[3]]),
            "music_id": u16::from_le_bytes([r[4], r[5]]),
            "status": r[6], "volume": r[7], "play_mode": r[8],
        }}),
        _ => json!({"success": true, "result": Value::Null}),
    }
}

async fn get_sd_play_name(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let to = ctx.timeout;

    match dev.send_command_and_wait(0x06, &[], to).await {
        Some(r) if r.len() >= 2 => {
            let n = u16::from_le_bytes([r[0], r[1]]) as usize;
            if r.len() >= 2 + n {
                json!({"success": true, "result": String::from_utf8_lossy(&r[2..2 + n])})
            } else {
                json!({"success": true, "result": Value::Null})
            }
        }
        _ => json!({"success": true, "result": Value::Null}),
    }
}

async fn get_sd_music_list(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
    let to = ctx.timeout;

    let start = kw_i64(kw, "start_id")
        .or_else(|| args.first().copied())
        .unwrap_or(0);
    let end = kw_i64(kw, "end_id")
        .or_else(|| args.get(1).copied())
        .unwrap_or(0);
    let mut req = Vec::new();
    req.extend_from_slice(&le16(start));
    req.extend_from_slice(&le16(end));
    dev.send_command_and_wait(0x07, &req, to).await.map_or_else(
        || json!({"success": true, "result": []}),
        |r| {
            let mut list = Vec::new();
            let mut off = 0usize;
            while off + 4 <= r.len() {
                let id = u16::from_le_bytes([r[off], r[off + 1]]);
                off += 2;
                let name_len = u16::from_le_bytes([r[off], r[off + 1]]) as usize;
                off += 2;
                if off + name_len <= r.len() {
                    let name = String::from_utf8_lossy(&r[off..off + name_len]).to_string();
                    list.push(json!({"id": id, "name": name}));
                    off += name_len;
                } else {
                    break;
                }
            }
            json!({"success": true, "result": list})
        },
    )
}
