...
pub async fn handle(method: &str, ctx: CallCtx<'_>) -> Value {
    ...
    match method {
        ...
        "hot_update.update" => update(ctx).await,
        "music.set_volume" | "set_volume" => set_volume(ctx).await,
        "music.get_volume" | "get_volume" => get_volume(ctx).await,
        "radio.set_radio_frequency" | "set_radio_frequency" | "radio.set_radio" | "set_radio" => {
            set_radio_frequency(ctx).await
        }
        "system.set_low_power_switch"
        | "device.set_low_power_switch"
        | "set_low_power_switch"
        | "device.set_low_power"
        | "set_low_power" => set_low_power_switch(ctx).await,
        "system.get_low_power_switch"
        | "device.get_low_power_switch"
        | "get_low_power_switch"
        | "device.get_low_power"
        | "get_low_power" => get_low_power_switch(ctx).await,
        "system.set_auto_power_off"
        | "device.set_auto_power_off"
        | "set_auto_power_off"
        | "sound.set_auto_power_off" => set_auto_power_off(ctx).await,
        "system.get_auto_power_off"
        | "device.get_auto_power_off"
        | "get_auto_power_off"
        | "sound.get_auto_power_off" => get_auto_power_off(ctx).await,
        "animation.stream_animation_8b" => stream_animation_8b(ctx).await,
        _ => err_reply("unimplemented basic command"),
    }
}
async fn get_device_name(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
...
async fn set_device_name(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
...
async fn get_brightness(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let timeout = ctx.timeout;
...
async fn get_light_mode(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let timeout = ctx.timeout;
...
async fn set_brightness(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
...
async fn set_screen_on(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let raw_args = ctx.raw_args;
    let kw = ctx.kwargs;
...
async fn show_hot_channel(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
...
async fn update(ctx: CallCtx<'_>) -> Value {
    let kw = ctx.kwargs;
...
async fn set_volume(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
...
async fn get_volume(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let timeout = ctx.timeout;
...
async fn set_radio_frequency(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
...
async fn set_low_power_switch(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let raw_args = ctx.raw_args;
    let kw = ctx.kwargs;
...
async fn get_low_power_switch(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let timeout = ctx.timeout;
...
async fn set_auto_power_off(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let args = ctx.args;
    let kw = ctx.kwargs;
...
async fn get_auto_power_off(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let timeout = ctx.timeout;
...
async fn stream_animation_8b(ctx: CallCtx<'_>) -> Value {
    let dev = ctx.dev;
    let kw = ctx.kwargs;
