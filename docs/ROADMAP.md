...
| P1.3 device commands | **MOSTLY DONE** | P1.3a: two gate false positives fixed, `live_job_start`/`live_job_stop` deleted. P1.3b: three of the six resolved from SOURCE, no hardware needed — `display_custom_art` (superseded by `open_file_dialog` -> `display_wall_image`, same `show_image` dispatch), `apply_system_stats` (one-shot push superseded by the `toggle_sysmon_sync` live job), `custom_art_query_page` (unused read-back; daemon keeps `cmd_custom_art_query_page`). **Three remain and are NOT superseded — they are UNEXPOSED features** (`set_clock_rich`, `set_temperature_channel`, `set_timeplan`); wire-or-delete needs device evidence -> P2.5. Allowlist 8 -> 5 |
...
| P1.6 tests + floor rebaseline | **DONE** | Floor **89.0 -> 89.4**; the owed trip back up is paid. Arc stated in `py_ci.sh`: 89.50 start -> 89.41 (P1.1) -> 88.99 (P1-P3) -> **89.48** final, after covering `play_album`/`push_playlist`, whose migration into the LAN funnel had left them tested only through e2e. Ends 0.02 BELOW the start having deleted ~430 statements of dead code — the ratio behaving as designed, not decay. 3002 passed / 94 skipped |
