...
- **2026-09-12 — keyboard session with the user: ALL SIX defects LIVE-CONFIRMED on the installed build; six more fleet defects found live and fixed.**
  ...
  - Both closed before the user went afk: now-playing empty-session fix (`64c7100`); signing identity created ("Divoom Local Signing", self-signed, trusted for codeSign in the login keychain), scripts sign with it, and a second different build installed WITHOUT a Bluetooth prompt (user-confirmed). The no-rebuild-while-away rule is retired: rebuilds are prompt-free on this machine now.
...
- **2026-09-12 — follow-ups SHIPPED: GUI names its panel (`dfb0c3f`), #2 animated previews (`c10af86`).**
  ...
  - NOT installed (Bluetooth prompt rule); `dist/` is behind these last two commits too -- rebuild + `install_local.sh` when present, then glance: bench animates on animated art; Clock/Custom Art panels stay populated; four panels stay connected while selecting.
  - Remaining and keyboard-bound: stable signing identity (roadmap), live glances #1 (cover art with music) / #6 (reconnect cycle).
- **2026-09-12 — live-widget "repeating" class ROOT-CAUSED and FIXED: per-device aggregate (`ea60483`, `355b0a6`).**
  ...
  - 7/7 stress, whole crate green both matrices, clippy pedantic both, menubar 20/20; `ci_local.sh --fast` 27/27.
  - LIVE on the real 4-panel fleet (traced dev daemon, socket-driven, wire log as evidence): sysmon frames flow; ZERO frames after `live_job_stop`; a mac-less `switch_channel` preempts the job with zero frames after; weather retires sysmon; two panels streaming while a third switches in 0.06s and both keep streaming; 10x churn leaves the panel silent; disconnect-all + reconnect-all lands zero live frames on any panel. One BLE flake: Timoo "not found in scan" on the first connect, fine on reconnect.
  - The trace's TX lines carried no device id (fixed as `d389be1`, not installed). The build in `dist/` has the aggregate; `/Applications/Divoom.app` is still the #5-fix build -- NOT reinstalled on purpose: each rebuilt ad-hoc-signed binary raises a new Bluetooth prompt and the user was away (roadmap item filed). Run `scripts/install_local.sh` when present (one prompt).
...
- **2026-09-12 — live session: 4 devices available, 1 connected (`f90d2cc9…`, BLE).**
  ...
  - Still needing eyes: #5 distinguishing check, #2 GIF motion, confirmations for #1/#4/#6.
- **2026-09-12 — #3a MEASURED live (transient switches, restored to clock): all channels 0.04–0.12s.**
  - No queue change — would be guessing against a 50ms measurement. Residual: wedged-link stalls + fixed #6 desync. Roadmap updated; probe script was throwaway (`/tmp`, removed).
