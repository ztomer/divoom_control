...
- **2026-09-21 — release train IN FLIGHT (v0.39.0 target): five commits from the uncommitted rounds + bump + tag + install.** Owner ordered the full train: CHANGELOG Unreleased stanza extended to all five changes (MCP negotiation, Python 3.14 floor, tray-icon trim, tao→winit, workflow comment); stale refs fixed (README 3.10+→3.14, ARCHITECTURE/README-tree/test-docstring tao→winit; 2024-11-05 kept only as supported-echo; no `needs GTK` left); no scratch/untracked files to prune; audit ignores already empty (to verify). Planned commits: (a) MCP negotiation, (b) Python floor, (c) tray-icon trim, (d) tao→winit, (e) workflow comment, then a version-bump commit (0.39.0: pyproject + both product crates + CHANGELOG rename + release notes). Then push, CI-green watch, release.sh, install_local.sh + daemon proof. Status: docs done, verification next.
